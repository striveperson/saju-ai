...
/**
 * 만 `age` 세가 되는 생일이 속한 사주 연도. docs/05 9.2.
 *
 * 출생 사주 연도에 나이를 더하는 산술로 대신하지 않는다.
 * 입춘 절입 시각이 해마다 흔들려 그 산술이 경계에서 한 해 밀린다.
 *
 * 절기 데이터 밖이면 `null` 이다. docs/05 9.3.
 */
...
export interface Daeun {
   ...
   *
   * 출생 사주 연도에 나이를 더하는 산술로 대신하지 않는다. docs/05 9.2.
   * 그 생일이 절기 데이터 밖이면 `null` 이다. 간지와 나이는 그대로 있다. docs/05 9.3.
   */
 ...
 * 입춘 시각이 흔들려 그 산술이 경계에서 어긋난다. docs/05 9.2.
 *
