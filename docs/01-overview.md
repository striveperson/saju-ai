...
계산과 판정은 클라이언트에서 실행된다. B 부터 D 까지는 네트워크를 타지 않는다.
로그인은 저장과 공유 시점에만 요구한다([ADR 0008](adr/0008-social-login-server-storage.md)).

A 단계는 서버를 거친다. 출생지가 필수 입력이고 그 경도를 주소 API 로 찾기 때문이다
([ADR 0019](adr/0019-region-lookup-via-address-api.md)).
[ADR 0003](adr/0003-spa-bundle-for-app.md) 3항이 약속한 오프라인 동작이 그만큼 좁아졌다.

## 4. 화면

| 화면 | 하는 일 | 로그인 |
| ---- | ------- | ------ |
| 입력 | 이름, 생년월일시, 성별, 출생지(경도) 입력. 계산 옵션은 나중에 | 불필요 |
