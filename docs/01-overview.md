...
| 설정 | 진태양시 보정, 야자시 정책 등 계산 옵션 기본값 | 불필요 |
...
| 옵션 | 기본값 | 갈리는 이유 |
| ---- | ------ | ----------- |
| 진태양시 보정 | 끔 | 상용 만세력 다수가 미적용이라 켜면 타 서비스와 결과가 달라진다 |
| 야자시 정책 | 제품 결정 사항 | 23시 이후 출생의 일주 귀속이 정자시설과 야자시설로 갈린다 |
| 지원 세력 범위 | 비겁 + 인성 | 득령·득지·득시를 비겁만으로 좁히는 유파가 있다 |
...
| 진태양시 | 眞太陽時 | `trueSolarTime` | 출생지 경도로 보정한 실제 태양 기준 시각 |
