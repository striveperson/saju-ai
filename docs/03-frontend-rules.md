...
- 지면 전용 컴포넌트를 다른 지면에서 가져오지 않는다.
  ...
  컴포넌트가 아닌 것은 `@shared` 로 올린다. 지면 사이로 넘기는 값의 타입과 스키마가 그것이다
...
히스토리 state 가 경계인 것은 브라우저가 새로고침 때 복원해 주기 때문이다.
...
지면 사이로 넘겨야 하면 히스토리 state 를 쓴다.
`navigate({ to, state })` 가 URL 이 아니라 히스토리 항목에 싣고
`@tanstack/history` 의 `HistoryState` 를 모듈 확장해 타입을 붙인다.

받는 쪽은 값이 없을 때를 반드시 다룬다.
서버에는 히스토리 state 가 없으므로 그 라우트를 직접 열거나 새로고침하면 비어서 온다.
`beforeLoad` 에서 검증하고 없으면 앞 지면으로 되돌린다.
서버가 리다이렉트로 응답하면 그 라우트의 HTML 이 나가지 않아
클라이언트가 복원한 state 와 어긋날 자리도 생기지 않는다.
