...
적용한 유파 옵션을 결과 화면에 항상 표시하는 것([01](01-overview.md) 5장),
...
경로 별칭은 넷이고 `apps/web/tsconfig.json` 의 `paths` 가 단일 소스다.
...
API 응답, URL 파라미터, `localStorage` 가 그 경계다.

검증 라이브러리는 zod 를 쓴다. 다만 `apps/web` 에 아직 설치되어 있지 않다.
화면 작업을 시작할 때 추가한다. 그 전까지 이 절의 검증 규칙은 적용할 대상이 없다.
...
- 화면 디자인과 경고 문구 체계는 아직 정하지 않았다. 루트 CLAUDE.md 가 그렇게 적고 있다
