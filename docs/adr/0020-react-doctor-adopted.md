...
5. 커밋 훅에 넣지 않는다. `pre-commit-check.sh` 는 그대로 넷이다.
6. 규칙 설정 파일을 만들지 않는다. 기본 규칙으로 돌리고, 오탐이 쌓이면 그때 정한다.
...
- 진단이 조언일 뿐이라 아무도 안 돌리면 없는 것과 같다.
  훅에 넣지 않기로 한 5항의 대가다. 커밋마다 4초를 더 쓰는 것과 바꾼 것인데,
  화면 코드가 더 늘고 나서 다시 본다.
